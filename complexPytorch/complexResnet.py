...
from __init__ import ComplexConv2d, NaiveComplexBatchNorm2d, ComplexReLU, ComplexMaxPool2d, ComplexAdaptiveAvgPool2d, ComplexLinear, CombineComplexLinear
...
class ResNet(nn.Module):
    def __init__(
        ...
        if self.real_output:
            self.fc = CombineComplexLinear((512 * block.expansion, num_classes))
        else:
        ...
        for m in self.modules():
            ...
            elif isinstance(m, (NaiveComplexBatchNorm2d, nn.GroupNorm)):
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, Bottleneck) and m.bn3.weight is not None:
                    nn.init.constant_(m.bn3.weight, 0+0j)  # type: ignore[arg-type]
                elif isinstance(m, BasicBlock) and m.bn2.weight is not None:
                    nn.init.constant_(m.bn2.weight, 0+0j)  # type: ignore[arg-type]
