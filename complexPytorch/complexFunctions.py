...
def complex_adaptive_avg_pool2d(inp, *args, **kwargs):
    """
    Perform complex adaptive average pooling.
    """
    absolute_value_real = adaptive_avg_pool2d(inp.real, *args, **kwargs)
    absolute_value_imag = adaptive_avg_pool2d(inp.imag, *args, **kwargs)
...
def complex_tanh(inp):
    return tanh(inp.real) + 1j * tanh(inp.imag)
...
def complex_gelu(inp):
    """Complex Gaussian Error Linear Units function element-wise, approximated with tanh.
    See https://arxiv.org/abs/1606.08415 for more details.
    """
    return 0.5 * inp * (1 + torch.tanh(torch.sqrt(torch.tensor(2 / torch.pi)) * (inp + 0.044715 * torch.pow(inp, 3))))
